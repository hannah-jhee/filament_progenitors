...
def read_filament(filename):
    if not os.path.isfile(filename):
        ...
        smoothing_weight = float(path_split[-1].split('_')[3][1:])
        nsig     = path_split[-1][path_split[-1].index('fits_c')+6:path_split[-1].index('.up')]
        smooth   = int(path_split[-1][path_split[-1].index('S')+1:path_split[-1].index('.rmB.a.NDskl')])

        run_disperse(basePath, snapNum, clsnum, smoothing_weight, nsig, smooth)#, assemble=None)"""
...
def run_disperse(basePath, snapNum, clsnum, smoothing_weight, nsig, smooth):
    """
    It runs DisPerSE : delaunay_3D -> mse -> skelconv.
    It only acts when a skeleton file doesn't exist.
    """
    filename = f"{basePath}/s{snapNum:03d}_c{clsnum:02d}_smt_w{smoothing_weight}_auto.fits"
    ...
    if os.path.isfile(f"{filename}"):
        ...
        subprocess.run(f"python /data2/disperse_smooth_test_N300/make_fits.py -snapNum {snapNum} -smoothing_weight {smoothing_weight} -smoothing_scale auto -nbins=300".split())
    ...
    if os.path.isfile(f"{filename}_c{nsig}.up.NDskl"):
        ...
        if os.path.isfile(f"{filename}.MSC"):
            subprocess.run(f"mse {filename} -cut {nsig} -upSkl -forceLoops -robustness -outDir /data2/disperse_smooth_test_N300/ -loadMSC {filename}.MSC".split())
        else:
            subprocess.run(f"mse {filename} -cut {nsig} -upSkl -forceLoops -robustness -outDir /data2/disperse_smooth_test_N300/".split())
    
    # skelconv
    subprocess.run(f"skelconv {filename}_c{nsig}.up.NDskl -smooth 10 -rmBoundary -outDir /data2/disperse_smooth_test_N300/ -to NDskl_ascii".split())
